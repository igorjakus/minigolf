...
namespace GML {
	...
	// ====================== Constructors ======================
	// ==========================================================
	Mat4f::Mat4f(float m00, float m10, float m20, float m30, float m01, float m11, float m21, float m31, float m02, float m12, float m22, float m32, float m03, float m13, float m23, float m33)
	...
	Mat4f::Mat4f(const Mat4f& A)
		: m00(A.m00), m10(A.m10), m20(A.m20), m30(A.m30), m01(A.m01), m11(A.m11), m21(A.m21), m31(A.m31), m02(A.m02), m12(A.m12), m22(A.m22), m32(A.m32), m03(A.m03), m13(A.m13), m23(A.m23), m33(A.m33) {}
	...
	Vec4f Mat4f::getIHat() const {
		return Vec4f(m00, m01, m02, m03);
	}
	Vec4f Mat4f::getJHat() const {
		return Vec4f(m10, m11, m12, m13);
	}
	Vec4f Mat4f::getKHat() const {
		return Vec4f(m20, m21, m22, m23);
	}
	Vec4f Mat4f::getWHat() const {
		return Vec4f(m30, m31, m32, m33);
	}
	...
	Mat4f Mat4f::transpose(const Mat4f& T) {
		return Mat4f(T.m00, T.m01, T.m02, T.m03, T.m10, T.m11, T.m12, T.m13, T.m20, T.m21, T.m22, T.m23, T.m30, T.m31, T.m32, T.m33);
	}
	...
	Mat4f Mat4f::multiply(const Mat4f& A, const Mat4f& B) {
		return Mat4f(A.m00 * B.m00 + A.m10 * B.m01 + A.m20 * B.m02 + A.m30 * B.m03,
